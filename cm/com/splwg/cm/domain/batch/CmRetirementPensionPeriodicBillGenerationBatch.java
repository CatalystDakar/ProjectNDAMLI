 ...
 */
public class CmRetirementPensionPeriodicBillGenerationBatch extends CmRetirementPensionPeriodicBillGenerationBatch_Gen {
	
	private final static Logger logger = LoggerFactory.getLogger(CmRetirementPensionPeriodicBillGenerationBatch.class);
	...
	private List<CmPeriodicBillEntity> getAllElements() {
		...
		String query = "SELECT TB.TAX_BILL_ID, PROC_FLOW_ID FROM C1_TAX_BILL TB, CI_PROC_FLOW_CHAR PCC "
				+ "WHERE TB.TAX_BILL_TYPE_CD = \'" + billType + "\' AND TB.BO_STATUS_CD = 'COMPLETED'"
				+ " AND PCC.CHAR_TYPE_CD = 'CM-RTPBL' AND TB.TAX_BILL_ID = PCC.CHAR_VAL_FK1 AND NOT EXISTS"
				+ " (SELECT PCFC.PROC_FLOW_ID FROM CI_PROC_FLOW_CHAR PCFC WHERE PCFC.PROC_FLOW_ID = PCC.PROC_FLOW_ID AND"
				+ " PCFC.CHAR_TYPE_CD='CM-RTPST' AND PCFC.CHAR_VAL='COMPLETED')";
	...
	public JobWork getJobWork() {
		...
		boolean isBillDate = checkBillingDate();
		if(isBillDate) {
	...
	private boolean checkBillingDate() {
		...
		boolean isBillDate = false;

		try {
			result = preparedStatement.iterate();
			isBillDate = result.hasNext();
		} catch (Exception exception) {
	...
	public Class<CmRetirementPensionPeriodicBillGenerationBatchWorker> getThreadWorkerClass() {
		return CmRetirementPensionPeriodicBillGenerationBatchWorker.class;
	}

	public static class CmRetirementPensionPeriodicBillGenerationBatchWorker extends CmRetirementPensionPeriodicBillGenerationBatchWorker_Gen {
		...
		public boolean executeWorkUnit(ThreadWorkUnit unit) throws ThreadAbortedException, RunAbortedException {
			...
			logger.info("Process Flow Id: " + element.getProcessFlowId());
			String processFlowId = element.getProcessFlowId();
			
			BusinessObjectInstance boInstance = null;
			boInstance=BusinessObjectInstance.create("CM-RetirementPensionPfwTransBO");
			boInstance.set("processFlowId", processFlowId);
			boInstance=BusinessObjectDispatcher.read(boInstance); 
			boolean isCertificateExists = verifyCertificate(processFlowId);
			billAmount = calculateBillAmount();
			TaxBill_Id taxBillId = new TaxBill_Id(element.getTaxBillId());			
			TaxBill_Id taxbBillId = null;
			if(isCertificateExists) {
				taxbBillId =  generateBill(taxBillId);
			} else {
				calculateBillAmount();
			}
		...
		private boolean verifyCertificate(String processFlowId) {
			// TODO Auto-generated method stub
			String query = "select count(*) BILL_STATUS from CI_PROC_FLOW_CHAR where CHAR_VAL = 'INPROGRESS'"
					+ " AND CHAR_TYPE_CD = 'CM-RTPST' AND PROC_FLOW_ID = \'" + processFlowId + "\'";
			PreparedStatement preparedStatement = createPreparedStatement(query, "SELECT");
			boolean isCertExist = true;
			...
			try {
				...
				if(count.intValue() == 6 || count.intValue() == 12 ) {
					isCertExist = lookForCertificate(processFlowId);
				}
		...
		private boolean lookForCertificate(String processFlowId) {
			// TODO Auto-generated method stub
			return true;
		}
			...
			try {
				...
				if(count.intValue() == 12) {
					ProcessFlowCharacteristic_Id pp1 = new ProcessFlowCharacteristic_Id(new ProcessFlow_Id(processFlowId),
							new CharacteristicType_Id("CM-RTPST"), count.add(BigInteger.ONE));
					ProcessFlowCharacteristic_Id pp = new ProcessFlowCharacteristic_Id(new ProcessFlow_Id(processFlowId),
							new CharacteristicType_Id("CM-RTPBL"), count.add(BigInteger.ONE));
