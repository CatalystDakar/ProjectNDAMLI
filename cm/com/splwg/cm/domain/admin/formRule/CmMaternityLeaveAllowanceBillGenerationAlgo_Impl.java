 ...
 * @author Anita M
 *
@AlgorithmComponent (softParameters = { @AlgorithmSoftParameter (name = calculationControlId, required = true, type = string)
 ...
 *            , @AlgorithmSoftParameter (name = filingCalendar, required = true, type = string)
 ...
 */
public class CmMaternityLeaveAllowanceBillGenerationAlgo_Impl extends CmMaternityLeaveAllowanceBillGenerationAlgo_Gen
		implements BusinessObjectExitStatusAlgorithmSpot {

	private static final Logger LOGGER = (Logger) LoggerFactory.getLogger( CmMaternityLeaveAllowanceBillGenerationAlgo_Impl.class );
	...
	public void invoke() {
		...
		LOGGER.info("***CmMaternityLeaveAllowanceBillGenerationAlgo_Impl***" + this.boInstance);
	    ...
	    COTSFieldDataAndMD<?> boStatusNode = this.boInstance.getFieldAndMDForPath("boStatus");
	    boStatus = boStatusNode.getValue().toString();
	    LOGGER.info("boStatus :: " + boStatus);
	    System.out.println("boStatus :: " + boStatus);
	    COTSFieldDataAndMD<?> pensionCategoryNode = this.boInstance.getFieldAndMDForPath("allowance/allowanceCategory");
	    ...
	    COTSFieldDataAndMD<?> cotsRegId1 = this.boInstance.getFieldAndMDForPath("trachepart1/billId");
	    COTSFieldDataAndMD<?> cotsRegId2 = this.boInstance.getFieldAndMDForPath("trachepart2/billIdt2");
	    COTSFieldDataAndMD<?> cotsRegId3 = this.boInstance.getFieldAndMDForPath("trachepart3/billIdt3");
	    COTSFieldDataAndMD<?> cotsRegId4 = this.boInstance.getFieldAndMDForPath("trachepart4/billIdt4");
	    COTSFieldDataAndMD<?> t1Amount = this.boInstance.getFieldAndMDForPath("trachepart1/montant");
	    COTSFieldDataAndMD<?> t2Amount = this.boInstance.getFieldAndMDForPath("trachepart2/montantt2");     
	    COTSFieldDataAndMD<?> t3Amount = this.boInstance.getFieldAndMDForPath("trachepart3/montantt3");
	    COTSFieldDataAndMD<?> t4Amount = this.boInstance.getFieldAndMDForPath("trachepart4/montantt4");
	    LOGGER.info("t1Amount :: " + t1Amount.getValue().toString());
	    if("CONTROLLER".equalsIgnoreCase(boStatus)){
	     amountStr = t1Amount.getValue().toString();
	    } else if("CONTROLET2".equalsIgnoreCase(boStatus)){
	     amountStr = t2Amount.getValue().toString();
	    } else if("CONTROLET3".equalsIgnoreCase(boStatus)){
		 amountStr = t3Amount.getValue().toString();
		} else if("CONTROLET4".equalsIgnoreCase(boStatus)){
		 amountStr = t4Amount.getValue().toString();
		}
	    amountStr = amountStr.replace("?","");
		System.out.println("Removed String : "+amountStr);
	    detailValue = new BigDecimal(amountStr);
		...
		if (!isBlankOrNull(personId)) {	
			...
			if(!isBlankOrNull(accountId)) {
				...
				FilingCalendar_Id filingCalender = new FilingCalendar_Id(this.getFilingCalendar());
				...
				valuationDetailTypeValue = valuationMap.get("IJCM");
				...
				//cotsRegId1.setXMLValue(taxbBillId.getEntity().getId().getIdValue());
				
				   if("CONTROLLER".equalsIgnoreCase(boStatus)){
					   cotsRegId1.setXMLValue(taxbBillId.getEntity().getId().getIdValue());
				    } else if("CONTROLET2".equalsIgnoreCase(boStatus)){
				    	cotsRegId2.setXMLValue(taxbBillId.getEntity().getId().getIdValue());
				    } else if("CONTROLET3".equalsIgnoreCase(boStatus)){
				    	cotsRegId3.setXMLValue(taxbBillId.getEntity().getId().getIdValue());
					} else if("CONTROLET4".equalsIgnoreCase(boStatus)){
						cotsRegId4.setXMLValue(taxbBillId.getEntity().getId().getIdValue());
					}
	...
	private Map<String, String> getLinkedMap(String[] valueArray) {
		...
		for (int i = 0; i < valueArray.length; i++) {
			...
			} else if(valueArray[i].contains("RETIRE")){
				calcMap.put("RETIRE", valueArray[i]);
			} else if(valueArray[i].contains("IJCM")){
				calcMap.put("IJCM", valueArray[i]);
			}
		}
		return calcMap;
	}
	...
	private TaxBill_Id generateBill(String taxRoleId, String obligationId) {
		...
		calcControlValue = calcControlMap.get("IJCM");
		LOGGER.info("calcControlValue:: " + calcControlValue +" billTypeMap:: " + billTypeMap.get("IJCM"));
		billDTO.setTaxBillTypeId(new TaxBillType_Id(billTypeMap.get("IJCM")));
		effectiveDateAllocationFam = getEffectiveDate("IJCM");
	...
	private Asset_Id getAssetId() {
		...
		assetType =  assetTypeMap.get("IJCM");
	...
	private String findOrCreateTaxRole(String accId, String serviceType, Asset_Id assetId) {
		...
		if(isBlankOrNull(taxRoleId)) {
			...
			serviceTypeValue =  serviceTypeMap.get("IJCM");
			serviceTypeValue = this.getServiceType();
	...
	private String findOrCreateObligation(String taxRoleId) {
		...
		if(isBlankOrNull(obligationId)) {		    
			...
			obligationTypeValue =  obligationTypeMap.get("IJCM");
